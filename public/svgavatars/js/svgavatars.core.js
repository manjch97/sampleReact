...
***************************************************************************************/
function loadAvatar(uploadSuccess,uploadFailure,selfUrl, backendUrl, enableConfirm) {
	"use strict";
	...
	if (window.svgAvatarsWPGlobals) { // WordPress plugin
		...
		saveUrl = options.backendUrl + 'php/save-ready-avatar.php';
		gravatarUrl = options.backendUrl + 'php/send-to-gravatar.php';
		WPNonce = '';
	}
			...
			'<div class="svga-col-right">',
				...
				'<div class="svga-row">',
					'<div id="svga-footermenu">',
						'<ul>',
							'<li id="svga-randomavatar" tabIndex="0" aria-label=" Random"><div></div><span class="svga-mobilehidden">',translation.randomMsg,'</span></li>',
							'<li id="svga-resetavatar" tabIndex="0" aria-label=" Reset"><div></div><span class="svga-mobilehidden">',translation.resetMsg,'</span></li>',
							'<li id="svga-saveavatar" tabIndex="0" aria-label=" Save"><div></div><span class="svga-mobilehidden">',translation.saveMsg,'</span></li>',
							'<li id="svga-shareavatar" tabIndex="0" aria-label=" Share"><div></div><span class="svga-mobilehidden">',translation.shareMsg,'</span></li>',
							'<li id="svga-gravataravatar" tabIndex="0" aria-label=" Gravatar"><div></div><span class="svga-mobilehidden">',translation.gravatarMsg,'</span></li>',
							'<li id="svga-downloadavatar" style="display:none">',
								//<div></div><span class="svga-mobilehidden">','Continue','</span>',
								// '<ul>',
	...
	console.log("URL====>",options.pathToFolder + 'json/svgavatars-male-data.json?ver=' + options.version);
	// gender is choosen
	$('#svga-start-boys').on('click', function() {
		...
		$.ajax({
			...
			if (options.debug) { console.log('JSON fail response: ', JSON.stringify(response)); }
	...
	function svgAvatars(gender, jsonData) {
		...
		for (var i = 0; i < blockNames.length; i++) {
			$('#svga-blocks').append('<div id="svga-blocks-'  + blockNames[i] + '" class="svga-blocks" data-blockname="' + blockNames[i] + '" tabIndex="0" aria-label=" block name">' +  translation.blockTitles[blockNames[i]] + '</div>');
		}
		...
		for (var currentZone in jsonData) {
			if (jsonData.hasOwnProperty(currentZone)) {
				$('#svga-bodyzones').append('<div id="svga-bodyzones-' + currentZone + '" class="svga-bodyzones" tabIndex="0" aria-label=" blocks sub" data-bodyzone="' + currentZone + '" data-controls="' + jsonData[currentZone].controls + '" data-block="' + jsonData[currentZone].block + '">' + translation.bodyZoneTitles[currentZone] + '</div>');
		...
		for (var cont in iconsSvgPathData) {
			if (iconsSvgPathData.hasOwnProperty(cont)) {
				if (controlNames.indexOf(cont) > -1) {
					$('#svga-controls').append('<div id="svga-controls-' + cont + '" class="svga-controls" tabIndex="0" aria-label=" Controls"><svg class="svga-control-icon" xmlns="http://www.w3.org/2000/svg" version="1.1" viewBox="0 0 16 16" preserveAspectRatio="xMinYMin meet"><path class="svga-control-icon-path" d="' + iconsSvgPathData[cont] + '"/></svg></div>');
		...
		for (cont in iconsSvgPathData) {
			if (iconsSvgPathData.hasOwnProperty(cont)) {
				if (globalControlNames.indexOf(cont) > -1) {
					$('#svga-glob-controls').append('<div id="svga-glob-controls-' + cont + '" class="svga-glob-controls" tabIndex="0" aria-label=" Icons"><svg class="svga-control-icon" xmlns="http://www.w3.org/2000/svg" version="1.1" viewBox="0 0 16 16" preserveAspectRatio="xMinYMin meet"><path class="svga-control-icon-path" d="' + iconsSvgPathData[cont] + '"/></svg></div>');
				}
			}
		}
		...
		for (i = 0; i < menuNames.length; i++) {
			if(menuNames[i] == 'download'){
				$('#svga-' + menuNames[i] + 'avatar > div').append('Confirm');
			}
		...
		for (currentZone in jsonData) {
			if (jsonData.hasOwnProperty(currentZone)) {
				...
				for (i = 0; i < jsonData[currentZone].shapes.length; i++) {
					$('#svga-elements-' + currentZone).append('<div class="svga-elements" id="svga-elements-' + currentZone + '-' + i + '" data-zone="' + currentZone + '" data-shape="' + i + '" tabIndex="0" aria-label=" elements features"></div>');
				...
				for (i = 0; i < currentColors.length; i++) {
					$('#svga-colors-' + currentZone).append('<div tabIndex="0" aria-label=" Color"></div>');
		...
		$('.svga-colors-set > div').on('click keypress', function() {
			var that = $(this),
				tempZones,
				currentColor;
		...
		$('.svga-bodyzones').on('click keypress', function() {
			var that = $(this);
		...
		$('.svga-elements').on('click keypress', function() {
			var that = $(this),
		...
		$('.svga-glob-controls').on('click keypress', function() {
			var id = $(this).attr('id'),
		...
		$('#svga-controls-up').on('click keypress', function() {
			var currentGroup;
			switch (bodyZone) {
				case 'mouth': {
		...
		$('#svga-controls-down').on('click keypress', function() {
			var currentGroup;
			switch (bodyZone) {
				case 'mouth': {
					currentGroup = SVG.get('svga-group-mouth-single-move');
		...
		$('#svga-controls-left').on('click keypress', function() {
			var currentGroup;
			switch (bodyZone) {
				case 'mouth': {
					currentGroup = SVG.get('svga-group-mouth-single-move');
		...
		$('#svga-controls-scaleup').on('click keypress', function() {
			var currentGroup;
			switch (bodyZone) {
				case 'faceshape': {
		...
		$('#svga-controls-scaledown').on('click keypress', function() {
			var currentGroup;
			switch (bodyZone) {
				case 'faceshape': {
					currentGroup = SVG.get('svga-group-faceshape-wrap');
		...
		$('#svga-controls-tightly').on('click keypress', function() {
			var currentGroup;
			switch (bodyZone) {
				case 'eyebrows': {
					currentGroup = SVG.get('svga-group-eyebrows-left-move');
		...
		$('#svga-controls-wider').on('click keypress', function() {
			var currentGroup;
			switch (bodyZone) {
				case 'eyebrows': {
					currentGroup = SVG.get('svga-group-eyebrows-left-move');
		...
		$('#svga-controls-eb1').on('click keypress', function() {
			var currentGroup;
		...
		$('#svga-controls-eb2').on('click keypress', function() {
			var currentGroup;
			currentGroup = SVG.get('svga-group-eyebrows-left-rotate');
		...
		$('#svga-controls-eb3').on('click keypress', function() {
			var currentGroup;
			currentGroup = SVG.get('svga-group-eyebrows-left-rotate');
			currentGroup.svgaCancelRotate().svgaRotateRight(1,rotateStep/2);
			currentGroup = SVG.get('svga-group-eyebrows-right-rotate');
		...
		$('#svga-controls-eb4').on('click keypress', function() {
			var currentGroup;
			currentGroup = SVG.get('svga-group-eyebrows-left-rotate');
		...
		$('#svga-controls-ebcancel').on('click keypress', function() {
			var currentGroup;
			currentGroup = SVG.get('svga-group-eyebrows-left-rotate');
		...
		function resetAvatar() {
			...
			$('#svga-colors-faceshape > div:nth-child(1)').trigger('click keypress');
			initZoom();
			changesCounter = 2;
			afterRandom = false;
		}
		...
		// click on reset
		$('#svga-resetavatar').on('click keypress', function() {
		...
		$('#svga-png-one').on('click', function() {
			...
			$('#svga-canvas').attr({width: 200, height: 200});
			$.ajax({
				url: options.backendUrl + "/e3-avatar-save",
				type: "post",
				dataType: 'json',
				data: {
					imgData: img,
		...
		$('#svga-downloadavatar').on('click', function() {
			...
			$('#svga-canvas').attr({width: 200, height: 200});
			uploadSuccess(img);
			//setTimeout$('#svga-work-overlay').fadeOut('fast');
			// $.ajax({
			// 	url: options.backendUrl + "/e3-avatar-save",
			// 	type: "POST",
			// 	dataType: 'json',
		...
		function getFile(tempFile, fileType) {
			var fD = new FileDownloader(options.backendUrl + 'e3-avatar-download');
	...
	function equalHeight() {
		...
		left.height('auto');
		right.height('auto');
		//console.log("window.innerWidth, left, right",window.innerWidth,left.height(),right.height())
...
function triggerConfirmAction(){
	if($('#svga-container').hasClass('svga-boys') || $('#svga-container').hasClass('svga-girls')){
		$("#svga-downloadavatar").trigger('click');
		return true;
	}
	else{
		return false;
